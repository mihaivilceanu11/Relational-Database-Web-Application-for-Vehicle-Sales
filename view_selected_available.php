...
<head>
	    <meta charset = "UTF-8">
	    <title> Selected Available Vehicles </title>
...
<?php
    ...
    <table class="center">
      ...
      session_start();
        ...
        $manufacturer_available = $_GET['manufacturer_available'];
        ...
        WHERE P.Denumire = '$manufacturer_available' AND A.ID_Auto NOT IN (SELECT ID_Auto FROM factura)";
...
<div>
<textarea rows="1" cols="40">You should contact:</textarea>
</div>
<?php
$conn2 = mysqli_connect('localhost', 'root', '', 'evidentavanzariautovehicule');
    $select2 = "SELECT DISTINCT AV.Nume, AV.Prenume, AV.Telefon, AV.Email
    FROM agentproducator AP JOIN agentvanzari AV on (AP.ID_Agent = AV.ID_Agent) JOIN autovehicul A ON (AP.ID_Producator = A.ID_Producator)
    JOIN producator P ON (P.ID_Producator = A.ID_Producator)
    WHERE P.Denumire = '$manufacturer_available' ";
...
<div>
  <textarea rows="1" cols="40"><?php echo $salesman_name; echo ' ';
      ...
      echo ', '; echo $Phone; echo', '; echo $Email;?>  </textarea>
  </div>
